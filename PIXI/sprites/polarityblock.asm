...
; when Mario touches the block, it will move into Mario's direction or away from him based on the on/off state
...
; $187B,X	=	direction (0 = right, 1 = left, 2 = up, 3 = down, FF = idle)
...
HandleDirection:
	LDA #$FF					; set the default direction to FF (idle)
	STA $187B,X
	
	LDA $1504,X					; if the block interaction flag is 1 (Mario is on top), set the direction to 2
	CMP #$01
	BNE +
	LDA #$02
	STA $187B,X
	...
	LDA $157C,X					; else, if the Mario interaction state is 2 (attached to side of block)...
	CMP #$02
	BNE +
	%SubHorzPos()				; move the block into Mario's direction
	TYA
	STA $187B,X
	BRA .moveblock
	+
	CMP #$03					; else, if the Mario interaction state is 3 (attached to bottom of block), set the direction to 3
	BNE +
	LDA #$03
	STA $187B,X
	BRA .moveblock
	...
	BRA HandleIdle				; else (Mario is not interacting with the block), don't move the block

.moveblock
	LDY $14AF					; invert the direction depending on the on/off state
	BEQ +
	EOR #$01
	+
...
Graphics:
	...
	PHY							; load tile YXPPCCCT properties based on the on/off state
	LDY $14AF
	LDA TileProp,Y
	ORA $64
	PLY
