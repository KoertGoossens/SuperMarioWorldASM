...
;	- bit 4:			kicked			+08		=	shell spawns in kicked state
...
; $187B,X	=	flag to indicate a sprite is inside the shell
...
.sethitbox
	...
	CMP #$09
	BEQ CarriableCode
	LDA $14C8,X					; if the sprite status was set to kicked, run the kicked code
	CMP #$0A
	BEQ KickedCode
...
HandleStunTimer:
	...
	LDA #$D0					; give the milde y speed
	STA $AA,Y
	
	PHX							; set the milde's x speed based on its horizontal position towards Mario
	PHY
	TYX
	%SubHorzPos()
	STY $00
	LDA MildeJumpSpeed,Y
	PLY
	PLX
	STA $B6,Y
	
	LDA $00						; set the milde's face direction based on its horizontal position towards Mario
	EOR #$01
	STA $157C,Y
	
	LDA #$01					; set the milde's knocked flag
	STA $C2,Y
...
Graphics:
	STZ $04						; set the stun timer x offset to 0
	
	LDA $14C8,X					; if the shell is not in kicked status...
	CMP #$0A
	BEQ +
	LDA $1540,X					; and the stun timer is low enough...
	CMP #$30
	BCS +
	AND #%00000010				; set the stun timer x offset to 1 pixel every 2 frames
	LSR
	STA $04
	+
...
; eyes tile
	LDA $187B,X					; if the shell has no sprite inside, don't draw eyes
	BEQ .skipeyes
...
HandleSpawnMilde:
	LDA $187B,X					; if the shell has a sprite inside...
	BEQ .return
	
	JSR SpawnMilde
	
	PHY							; set the milde's x speed based on the shell's horizontal position towards Mario
	%SubHorzPos()
	STY $00
	LDA MildeKnockSpeed,Y
	PLY
	STA $B6,Y
	
	LDA $00						; set the milde's face direction based on the shell's horizontal position towards Mario
	STA $157C,Y
	
	LDA $1504,X					; set the stored x speed for the milde
	STA $1594,Y
	PHX
	TYX
	STA $7FAB40,X
	...
	LDA #$08					; set the 'disable contact with Mario' timer for the milde
	STA $154C,Y
...
SpawnMilde:
	LDA #$10					; play magic sfx
	STA $1DF9
	
	STZ $1540,X					; set the shell's stun timer to 0
	STZ $187B,X					; clear the shell's 'sprite inside' flag
	
	LDA #$0A					; spawn a milde
	%SpawnCustomSprite()
	...
	LDA #$18					; set the 'disable entering shells' timer for the milde
	STA $1540,Y
...
DoKillSprite:
	...
	CMP #$0B
	BEQ .checkwearshell
	CMP #$0A						; if the indexed sprite is a milde, return
	BEQ .return
